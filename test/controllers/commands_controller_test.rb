...
class CommandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create command" do
    assert_difference('Command.count') do
      post commands_url, params: { command: { dog_id: @command.dog_id, name: @command.name } }
    end
  ...
  test "should update command" do
    patch command_url(@command), params: { command: { dog_id: @command.dog_id, name: @command.name } }
